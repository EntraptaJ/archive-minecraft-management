...
const HomeRoute: React.FunctionComponent<RouteComponentProps> = () => {
  const { data, loading } = useQuery<{ getMCStatus: MCStatus }>(MCSTATUSGQL);
  if (loading) return <div>Loading</div>;
  if (data && data.getMCStatus)
    return (
      <Layout>
        <div style={FormStyle}>
          <Typography use='headline4'>Minecraft Server</Typography>
          <Button onClick={() => navigate('/mods')} raised label='Mods' />
          {data.getMCStatus.players.length > 0 ? (
            <List>
              {data.getMCStatus.players.map(user => (
